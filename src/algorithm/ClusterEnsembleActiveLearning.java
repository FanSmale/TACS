package algorithm;

import java.util.Arrays;

import common.SimpleTools;
import exception.DuplicateQueryException;
import exception.LabelUsedUpException;
import exception.UnableToClusterInKException;

/**
 * Cluster ensemble based active learning.
 * <p>
 * Author: <b>Fan Min</b> minfanphd@163.com, minfan@swpu.edu.cn <br>
 * Copyright: The source code and all documents are open and free. PLEASE keep
 * this header while revising the program. <br>
 * Organization: <a href=http://www.fansmale.com/>Lab of Machine Learning</a>,
 * Southwest Petroleum University, Chengdu 610500, China.<br>
 * Project: The cost-sensitive active learning project.
 * <p>
 * Progress: Almost finished, further revision is possible.<br>
 * Written time: July 20, 2019. <br>
 * Last modify time: July 21, 2019.
 */

public class ClusterEnsembleActiveLearning extends
		ClusteringAlgorithmsBasedActiveLearning {

	/**
	 ********************
	 * The constructor.
	 * 
	 * @param paraFilename
	 *            The given file.
	 * @param paraDistanceMeasure
	 *            The given distance measure in integer.
	 * @param paraNormalizeData
	 *            Normalize data or not.
	 * @param paraAdaptiveRatio
	 *            The distance ratio for density computing.
	 * @param paraSmallBlockThreshold
	 *            Small block threshold.
	 * @param paraInstanceSelectionStrategy
	 *            Critical instance selection strategy.
	 * @param paraQueryAmountStrategy
	 *            Query enough instances at a time, or one by one.
	 * @param paraDisorderData
	 *            Disorder data or not.
	 ********************
	 */
	public ClusterEnsembleActiveLearning(String paraFilename,
			int paraDistanceMeasure, boolean paraNormalizeData,
			boolean paraDisorderData, double paraAdaptiveRatio,
			int paraSmallBlockThreshold, int paraInstanceSelectionStrategy,
			int paraQueryAmountStrategy) {
		super(paraFilename, paraDistanceMeasure, paraNormalizeData,
				paraDisorderData, paraAdaptiveRatio, paraSmallBlockThreshold,
				paraInstanceSelectionStrategy, paraQueryAmountStrategy);
	}// Of the constructor

	/**
	 ************************* 
	 * Learn. The most important process. This method takes advantage of a queue
	 * for unfinished blocks.
	 ************************* 
	 */
	public String learn() {
		String resultMessage = "";
		try {
			learnBlock(wholeBlock);
		} catch (Exception ee) {
			resultMessage += ee;
		}// Of try
		
		int tempNumUnhandled = getNumUnhandled();
		System.out.println("tempNumUnhandled = " + tempNumUnhandled);

		oneNnUnhandled(wholeBlock);

		resultMessage += "Clustering algorithm index = "
				+ currentClusteringAlgorithmIndex;
		resultMessage += ": " + currentClusteringAlgorithm;
		resultMessage += ", queries = " + getNumQueries(); // numQueries
		resultMessage += ", accuracy = " + computeAccuracy(); // accuracy
		resultMessage += ", final number of blocks = " + finalNumBlocks;
		// resultMessage += ", number of pure blocks = " + tempNumPureBlocks;
		// resultMessage += ", pure blocks size sum = " + tempPureBlocksSizeSum;
		resultMessage += ", number of small blocks = " + numSmallBlocks;
		resultMessage += ", misclassified = " + getNumMisclassified();
		resultMessage += ", misclassified in pure blocks = "
				+ getNumMisclassified(PURE_BLOCK_PREDICTED);
		resultMessage += ", misclassified by kNN = "
				+ getNumMisclassified(KNN_PREDICTED);
		resultMessage += ", misclassified by default label = "
				+ getNumMisclassified(DEFAULT_LABELED);
		resultMessage += ", numDefaultLabeled = "
				+ getNumInstancesByStatus(DEFAULT_LABELED);
		resultMessage += ", unhandled = "
				+ Arrays.toString(getInstancesByStatus(UNHANDLED));

		return resultMessage;
	}// Of learn

	/**
	 ************************* 
	 * Learn. The main process.
	 * 
	 * @param paraBlock
	 *            The current block.
	 * @throws LabelUsedUpException
	 *             If labels are used up.
	 * @throws DuplicateQueryException
	 *             If an instance has been queried multiple times.
	 ************************* 
	 */
	public void learnBlock(int[] paraBlock) throws LabelUsedUpException,
			DuplicateQueryException {
		// Step 1. Select instances to label.
		// May change later!
		System.out
				.println("ClusterEnsembleActiveLearning\r\nHandling a block with "
						+ paraBlock.length + " instances.");
		int tempNumInstancesToLabel = (int) Math.sqrt(paraBlock.length);
		selectCriticalAndLabel(paraBlock, tempNumInstancesToLabel);

		// Step 2. Classify if pure.
		boolean tempPure = true;
		int tempFirstLabel = -1;
		int tempFirstLabelIndex = -1;
		for (int i = 0; i < paraBlock.length; i++) {
			if (instanceStatusArray[paraBlock[i]] == QUERIED) {
				// if (isQueried(paraBlock[i])) {
				tempFirstLabel = predicts[paraBlock[i]];
				tempFirstLabelIndex = i;
				break;
			}// Of if
		}// Of for i

		for (int i = tempFirstLabelIndex + 1; i < paraBlock.length; i++) {
			if (instanceStatusArray[paraBlock[i]] == QUERIED) {
				if (predicts[paraBlock[i]] != tempFirstLabel) {
					tempPure = false;
					break;
				}// Of if
			}// Of if
		}// Of for i

		if (tempPure) {
			int tempPredicted = 0;
			// Now classify
			for (int i = 0; i < paraBlock.length; i++) {
				if (instanceStatusArray[paraBlock[i]] == QUERIED) {
					continue;
				} else {
					predicts[paraBlock[i]] = tempFirstLabel;
					instanceStatusArray[paraBlock[i]] = PURE_BLOCK_PREDICTED;
					tempPredicted++;
				}// Of if
			}// Of for i
			System.out.println("Pure block with " + paraBlock.length
					+ " instances and " + tempPredicted + " predicted as "
					+ tempFirstLabel);
			System.out.print("Queried: ");
			for (int i = 0; i < paraBlock.length; i++) {
				if (instanceStatusArray[paraBlock[i]] == QUERIED) {

					// if (isQueried(paraBlock[i])) {
					System.out.print(paraBlock[i] + "("
							+ data.instance(paraBlock[i]).value(numConditions)
							+ "), ");
				}// Of if
			}// Of for i
			System.out.println();
			System.out.println("Whole block" + Arrays.toString(paraBlock));

			finalNumBlocks++;

			return;
		}// Of if

		// Step 3. Pre-clustering and obtain the set family through
		// intersection.
		int[][] tempCurrentIntersections = null;
		int[][] tempFinalBlocks = null;
		int[][] tempCurrentAlgorithmBlocks = null;
		boolean tempFirstAlgorithm = true;

		for (int i = 0; i < NUM_ALGORITHMS; i++) {
			// Is this algorithm available?
			if (!availableAlgorithms[i]) {
				continue;
			}// Of if
			
			System.out.println("Algorithm #" + i);

			if (tempFirstAlgorithm) {
				try {
					tempFinalBlocks = clusterInTwo(paraBlock, i);
				} catch (UnableToClusterInKException ee) {
					continue;
				}// Of try

				tempFirstAlgorithm = false;
				continue;
			}// Of if

			try {
				tempCurrentAlgorithmBlocks = clusterInTwo(paraBlock, i);
			} catch (UnableToClusterInKException ee) {
				continue;
			}// Of try

			System.out.println("Algorithm #" + i + " success");
			
			// Try to intersect
			System.out.println("\r\n Compute intersection");

			System.out.println(Arrays.deepToString(tempFinalBlocks));
			System.out.println("intersects");
			System.out.println(Arrays.deepToString(tempCurrentAlgorithmBlocks));

			tempCurrentIntersections = SimpleTools
					.binarySetFamilyMaximalIntersection(tempFinalBlocks,
							tempCurrentAlgorithmBlocks);
			double tempOringialSize = tempFinalBlocks[0].length
					+ tempFinalBlocks[1].length;
			double tempNewSize = tempCurrentIntersections[0].length
					+ tempCurrentIntersections[1].length;
			double tempFraction = tempNewSize / tempOringialSize;
			System.out.println("\r\ntempFraction = " + tempFraction);
			if (tempFraction > 0.8) {
				System.out.println("adopt the new blocks.");
				tempFinalBlocks = tempCurrentIntersections;
			}// Of if
		}// Of for i

		System.out.println("Splitting a block with " + paraBlock.length
				+ " instances.\r\nThe new blocks are: ");
		System.out.println(Arrays.deepToString(tempFinalBlocks));

		if (tempFinalBlocks == null) {
			return;
		}// Of if

		// Step 4. Learn these two blocks.
		learnBlock(tempFinalBlocks[0]);
		learnBlock(tempFinalBlocks[1]);
	}// Of learnBlock

	/**
	 ************************* 
	 * Classify unhandled instances using kNN.
	 ************************* 
	 * public void knnUnhandled(int paraK) { int[] tempOriginalPredicts = new
	 * int[predicts.length]; for (int i = 0; i < tempOriginalPredicts.length;
	 * i++) { tempOriginalPredicts[i] = predicts[i]; }// Of for i
	 * 
	 * int[] tempNearestArray = new int[paraK + 2]; double[] tempDistanceArray =
	 * new double[paraK + 2]; double tempDistance;
	 * 
	 * int[] tempUnhandledArray = getInstancesByStatus(UNHANDLED); if
	 * (tempUnhandledArray == null) { return; }// Of if
	 * 
	 * for (int i = 0; i < tempUnhandledArray.length; i++) {
	 * Arrays.fill(tempNearestArray, -1); Arrays.fill(tempDistanceArray,
	 * Double.MAX_VALUE); tempDistanceArray[0] = -1; for (int j = 0; j <
	 * tempOriginalPredicts.length; j++) { if (tempOriginalPredicts[j] ==
	 * UNHANDLED) { continue; }// Of if
	 * 
	 * tempDistance = distanceMeasure.distance(tempUnhandledArray[i], j); for
	 * (int k = paraK; k >= 0; k--) { if (tempDistance < tempDistanceArray[k]) {
	 * tempDistanceArray[k + 1] = tempDistanceArray[k]; tempNearestArray[k + 1]
	 * = tempNearestArray[k]; } else { tempDistanceArray[k + 1] = tempDistance;
	 * tempNearestArray[k + 1] = j; }// Of if }// Of for k }// Of for j
	 * 
	 * // Vote to predict. int[] tempVoteArray = new
	 * int[data.attribute(numConditions) .numValues()]; for (int j = 1; j <=
	 * paraK; j++) { tempVoteArray[predicts[tempNearestArray[j]]]++; }// Of for
	 * j
	 * 
	 * int tempMaximalValue = -1; int tempMaximalIndex = -1; for (int j = 0; j <
	 * tempVoteArray.length; j++) { if (tempMaximalValue < tempVoteArray[j]) {
	 * tempMaximalValue = tempVoteArray[j]; tempMaximalIndex = j; }// Of if }//
	 * Of for j
	 * 
	 * predicts[tempUnhandledArray[i]] = tempMaximalIndex; }// Of for i }// Of
	 * knnUnhandled
	 */

}// Of ClusterEnsembleActiveLearning
