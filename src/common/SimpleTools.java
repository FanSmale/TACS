...
public class SimpleTools extends Object {
	 ...
	 ********************************** 
	 */
	public static String[] stringToStringArray(String paraString)
			throws Exception {
		/*
	...
	public static String[] parseString(String paraString, String paraDelimiter,
		...
		if (paraString != null) {
			token = new StringTokenizer(paraString.trim(), paraDelimiter,
					paraReturnTokens);
	...
	public static char subSetCheck(long paraFirstSet, long paraSecondSet,
		...
		int[] firstSubset = SimpleTools.longToIntArray(paraFirstSet,
				paraNumberOfConditions);
		int[] secondSubset = SimpleTools.longToIntArray(paraSecondSet,
				paraNumberOfConditions);
	...
	public static long[] removeSupersets(long[] paraSets,
		...
		for (int i = 0; i < paraSets.length - 1; i++) {
			...
			for (int j = i + 1; j < paraSets.length; j++) {
				...
				check = SimpleTools.subSetCheck(paraSets[i], paraSets[j],
						paraNumberOfConditions);
				if (check == '1') {
					paraSets[i] = -1;
					break;
	...
	public static int intersectionSize(int[] paraFirstSet, int[] paraSecondSet) {
		...
		while ((tempFirstIndex < paraFirstSet.length)
				&& (tempSecondIndex < paraSecondSet.length)) {
	 ...
	 ************************* 
	 */
	public static int[][] binarySetFamilyMaximalIntersection(
			int[][] paraFirstSetFamily, int[][] paraSecondSetFamily) {
		int[][] resultSetFamily = new int[2][];
		int tempIntersectionSize = intersectionSize(paraFirstSetFamily[0],
				paraSecondSetFamily[0]);

		if (tempIntersectionSize > paraFirstSetFamily[0].length / 2) {
			// A1 cap B1, A2 cap B2
			resultSetFamily[0] = intersection(paraFirstSetFamily[0],
					paraSecondSetFamily[0]);
			resultSetFamily[1] = intersection(paraFirstSetFamily[1],
					paraSecondSetFamily[1]);
		} else {
			// A1 cap B2, A2 cap B1
			resultSetFamily[0] = intersection(paraFirstSetFamily[0],
					paraSecondSetFamily[1]);
	...
	public static void normalizeDecisionSystem(Instances paraData) {
		...
		for (int i = 0; i < paraData.numAttributes() - 1; i++) {
			...
			if (tempMax == tempMin) {
				System.out
						.println("Fatal error occurred in SimpleTools.normalize()! Attribute #"
			...
			for (int j = 0; j < paraData.numInstances(); j++) {
				tempNewValue = (paraData.instance(j).value(i) - tempMin)
						/ (tempMax - tempMin);
	...
	public static void disorderData(String paraFilename) {
		...
		String tempNewFilename = paraFilename.substring(0, tempLength - 5)
				+ "_disorder.arff";
		System.out.println(tempNewFilename);
		try {
			RandomAccessFile tempNewFile = new RandomAccessFile(
					tempNewFilename, "rw");
	...
	/**
	 ********************************** 
	 * Sample a dataset with the given fraction.
	 * 
	 * @param paraFilename
	 *            The given filename.
	 * @para paraFraction The given fraction.
	 ********************************** 
	 */
	public static void sampleData(String paraFilename, double paraFraction) {
		// Step 1. Read the data.
		Instances tempData = null;
		try {
			FileReader fileReader = new FileReader(paraFilename);
		...
		// Step 3. Count the number of instances in the sampled data.
		int tempNumSamples = (int) (tempData.numInstances() * paraFraction);
	...
	public static void testInBlock() {
		...
		System.out.println("" + tempInt + " in the block "
				+ Arrays.toString(tempBlock) + "? " + tempInBlock);
	...
	public static void testBinarySetFamilyMaximalIntersection() {
		...
		int[][] resultFamily = binarySetFamilyMaximalIntersection(
				tempFirstSetFamily, tempSecondSetFamily);
		System.out.println("The result set family is: "
				+ Arrays.deepToString(resultFamily));
	...
	public static void main(String args[]) {
		...
		sampleData("src/data/DLA.arff", 0.05);
	}// Of main
}// Of class SimpleTool
